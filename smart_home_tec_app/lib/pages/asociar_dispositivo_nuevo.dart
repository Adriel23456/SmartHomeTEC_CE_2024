...
class _AsociarDispositivoNuevo extends State<AsociarDispositivoNuevo> {
  ...
  bool _deviceFieldsFilled() {
    if (serialNumber.text.isNotEmpty &&
        deviceType.text.isNotEmpty &&
        description.text.isNotEmpty &&
        brand.text.isNotEmpty &&
        consumption.text.isNotEmpty &&
        chamberName.text.isNotEmpty) {
      return true;
    } else {
      return false;
    }
      ...
      if (_checkSerialInt()) {
        if (await _checkChamberName()) {
          if (await _checkDeviceTypeExistance()) {
            var res = await db.createDispositivo(
                AssignedDevice(
